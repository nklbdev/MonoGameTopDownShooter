    ...
    public class Polygon
    {
        ...
        public static Polygon FromPoints(IEnumerable<Point> points)
        {
            ...
            var poly = new Polygon
            {
                ...
                Bounds =
                {
                    X = pointArray.Min(x => x.X),
                    Y = pointArray.Min(x => x.Y),
                    Width = pointArray.Max(x => x.X) - pointArray.Min(x => x.X),
                    Height = pointArray.Max(x => x.Y) - pointArray.Min(x => x.Y)
                }
            };
        ...
        {
            var colorData = new Color[Bounds.Width * Bounds.Height];
            ...
            Texture = new Texture2D(graphicsDevice, Bounds.Width, Bounds.Height, false, SurfaceFormat.Color);
        ...
        public void DrawFilled(IImmutableSpriteBatch spriteBatch, Rectangle region, Texture2D texture, float lineWidth, Color color, Color fillColor, float layerDepth)
        {
            ...
            spriteBatch.Draw(Texture, Map.Translate(Bounds, region), null, fillColor, 0, Vector2.Zero, SpriteEffects.None, layerDepth);
        }
        ...
        public bool Contains(ref Vector2 vector)
        {
            ...
            foreach (var side in Lines)
            {
                ...
                var xIntersection = (vector.Y - side.Start.Y) * (side.End.X - side.Start.X) / (side.End.Y - side.Start.Y) + side.Start.X;
        ...
        {
            var pointsContained = ((Contains(new Vector2(rect.Left, rect.Top)) ? 1 : 0) +
        ...
        {
            var pointsContained = ((Contains(line.Start) ? 1 : 0) + (Contains(line.End) ? 1 : 0));
